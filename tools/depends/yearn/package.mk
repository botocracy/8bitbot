...
$(S)/build-yearn: $(S)/.prebuild $(YEARN_BUILD_DEPENDS)
	...
	patch \
	  ...
	  < "$(TOOL_DIR)/depends/yearn/$(YEARN_VERSION)/0001-Delegate-OpenZeppelin-versioning-to-dependency-manag.patch" \
	  || :

ifeq ($(YEARN_VERSION),develop)
	patch \
	  -p1 \
	  --forward \
	  --directory="$(BUILD_DIR_YEARN)" \
...
$(S)/test-yearn: $(S)/.preinstall $(YEARN_TEST_DEPENDS)
	...
	[ ! -d "$(BUILD_DIR_YEARN)/tests/functional" ] || ( \
	  cd "$(BUILD_DIR_YEARN)" && \
	  source bin/activate && \
	  brownie test -s \
	)

...
{
  /*
  std::cout << "Initializing decoder for \"" << m_fileName << "\""
      << std::endl;
  */
  ...
  m_pFormatContext->iformat = inputFormat;
  ...
  // Reduce destination size to fit constraint
...
void StreamDecoder::Decode()
{
  ...
  if (packet.stream_index == m_videoStreamId)
  {
    ...
    if (frameFinished != 0)
    {
      ...
      sws_scale(m_scaler, m_decodedFrame->data, m_decodedFrame->linesize,

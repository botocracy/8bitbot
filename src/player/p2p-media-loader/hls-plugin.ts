...
function hlsjsConfigHandler(
  this: videojs.Player,
  options: HlsjsConfigHandlerOptions
) {
  const player = this;
...
class Html5Hlsjs {
  ...
  private handlers: {
    [id in
      ...
      | 'playing'
      | 'textTracksChange'
      | 'audioTracksChange']: EventListener;
  } = {
    play: null,
    addtrack: null,
    playing: null,
  ...
  dispose() {
    ...
    this.videoElement.textTracks.removeEventListener(
      'addtrack',
      this.handlers.addtrack
    );
    this.videoElement.removeEventListener('playing', this.handlers.playing);

    this.player
      .textTracks()
      .removeEventListener('change', this.handlers.textTracksChange);
  ...
  private _handleQualityLevels() {
    ...
    const qualityLevels =
      this.player.qualityLevels && this.player.qualityLevels();
    ...
    for (let i = 0; i < this.metadata.levels.length; i++) {
      ...
      const self = this;
      representation.enabled = function (
        this: QualityLevels,
        level: number,
        toggle?: boolean
      ) {
  ...
  private _onAudioTracks() {
    ...
    if (hlsAudioTracks.length > 1 && playerAudioTracks.length === 0) {
      ...
      this.handlers.audioTracksChange = this._updateSelectedAudioTrack.bind(
        this
      );
      playerAudioTracks.addEventListener(
        'change',
        this.handlers.audioTracksChange
      );
    }
  }
  ...
  private _isSameTextTrack(track1: TextTrack, track2: TextTrack) {
    return (
      this._getTextTrackLabel(track1) === this._getTextTrackLabel(track2) &&
  ...
  private _updateSelectedTextTrack() {
    ...
    for (let k = 0; k < hlsjsTracks.length; k++) {
      if (
        ...
        hlsjsTracks[k].mode =
          activeTrack && this._isSameTextTrack(hlsjsTracks[k], activeTrack)
            ? 'showing'
            : 'disabled';
      }
    }
  ...
  private _updateTextTrackList() {
    const displayableTracks = this._filterDisplayableTextTracks(
    ...
    for (let idx = 0; idx < displayableTracks.length; idx++) {
      ...
      for (let jdx = 0; jdx < playerTextTracks.length; jdx++) {
        if (
          this._isSameTextTrack(displayableTracks[idx], playerTextTracks[jdx])
        ) {
          isAdded = true;
          break;
        }
    ...
    if (!this.uiTextTrackHandled) {
      ...
      playerTextTracks.addEventListener(
        'change',
        this.handlers.textTracksChange
      );
  ...
  private _initHlsjs() {
    ...
    const hlsjsConfigRef =
      (srOptions_ && srOptions_.hlsjsConfig) || techOptions.hlsjsConfig;
    ...
    const captionConfig =
      (srOptions_ && srOptions_.captionConfig) || techOptions.captionConfig;
    if (captionConfig) {
    ...
    this.hls.on(Hlsjs.Events.ERROR, (event, data) =>
    ...
    this.hls.on(Hlsjs.Events.MANIFEST_PARSED, (event, data) =>
      this._onMetaData(event, data as any)
    ); // FIXME: typings
...
export { Html5Hlsjs, registerSourceHandler, registerConfigPlugin };

...
class P2pMediaLoaderPlugin extends Plugin {
  ...
  constructor(player: videojs.Player, options?: P2PMediaLoaderPluginOptions) {
    ...
    if (!(videojs as any).Html5Hlsjs) {
      console.warn(
        'HLS.js does not seem to be supported. Try to fallback to built in HLS.'
      );
      ...
      // FIXME: typings https://github.com/Microsoft/TypeScript/issues/14080
      (videojs as any).Html5Hlsjs.addHook(
        'beforeinitialize',
        (videojsPlayer: any, hlsjs: any) => {
          this.hlsjs = hlsjs;
        }
      );
  ...
  private initializePlugin() {
    ...
    this.statsP2PBytes.numPeers =
      1 + this.options.redundancyUrlManager.countBaseUrls();
  ...
  private runStats() {
    this.p2pEngine.on(
      Events.PieceBytesDownloaded,
      (method: string, size: number) => {
        const elem =
          method === 'p2p' ? this.statsP2PBytes : this.statsHTTPBytes;

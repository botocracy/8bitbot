...
export class ChunkStore extends EventEmitter {
    ...
    this.putBulkTimeout = setTimeout(async () => {
      ...
      try {
        await this.db.transaction('rw', this.db.chunks, () => {
          return this.db.chunks.bulkPut(
            processing.map((p) => ({ id: p.id, buf: p.buf }))
          );
        });
      } catch (err) {
  ...
  get(index: number, opts: any, cb: (err?: Error, buf?: Buffer) => void): void {
    ...
    // Chunk in store
    this.db
      .transaction('r', this.db.chunks, async () => {
  ...
  private runCleaner() {
    ...
    this.cleanerInterval = setInterval(async () => {
      this.checkExpiration();
    }, ChunkStore.CLEANER_INTERVAL_MS);
  }
  ...
  private async checkExpiration() {
    ...
    try {
      await this.expirationDB.transaction(
        'rw',
        this.expirationDB.databases,
        async () => {
          // Update our database expiration since we are alive
          await this.expirationDB.databases.put({
            name: this.databaseName,
            expiration: new Date().getTime() + ChunkStore.CLEANER_EXPIRATION_MS,
          });

          const now = new Date().getTime();
          databasesToDeleteInfo = await this.expirationDB.databases
            .where('expiration')
            .below(now)
            .toArray();
        }
  ...
  private async dropDatabase(databaseName: string) {
    ...
    try {
      ...
      await this.expirationDB.transaction(
        ...
        this.expirationDB.databases,
        () => {
          return this.expirationDB.databases

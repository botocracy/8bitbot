...
import { PeertubeChunkStore } from './chunk-store';
import {
...
class WebTorrentPlugin extends Plugin {
  ...
  updateVideoFile(
    ...
    if (
      ...
      this.currentVideoFile.magnetUri === videoFile.magnetUri
    ) {
      return;
    }
    ...
    this.addTorrent(
      this.currentVideoFile.magnetUri,
      previousVideoFile,
      options,
      () => {
        this.player.playbackRate(oldPlaybackRate);
  ...
  updateResolution(resolutionId: number, delay = 0) {
    ...
    const newVideoFile = this.videoFiles.find(
      (f) => f.resolution.id === resolutionId
    );
    const options = {
      forcePlay: false,
      delay,
  ...
  private addTorrent(
    ...
    const torrentOptions = {
      ...
      store: function (chunkLength: number, storeOpts: any) {
        return new CacheChunkStore(
          new PeertubeChunkStore(chunkLength, storeOpts),
          {
            max: 100,
          }
        );
    ...
    const filteredFiles = files
      ...
      .filter((f) => {
        ...
        if (
          ...
          f.resolution.id > this.currentVideoFile.resolution.id
        ) {
          threshold +=
            (fileBitrate * this.CONSTANTS.AUTO_QUALITY_THRESHOLD_PERCENT) / 100;
        }
  ...
  private getAndSaveActualDownloadSpeed() {
    const start = Math.max(
      this.downloadSpeeds.length -
        this.CONSTANTS.BANDWIDTH_AVERAGE_NUMBER_OF_VALUES,
      0
    );
    const lastDownloadSpeeds = this.downloadSpeeds.slice(
      start,
      this.downloadSpeeds.length
    );
  ...
  private runAutoQualityScheduler() {
    this.autoQualityInterval = setInterval(() => {
      ...
      if (
        ...
        file.resolution.id < this.currentVideoFile.resolution.id
      ) {
        console.log(
          'Downgrading automatically the resolution to: %s',
        ...
        // Higher resolution
        console.log(
          'Upgrading automatically the resolution to: %s',
          file.resolution.label
        );
        changeResolution = true;
        changeResolutionDelay = this.CONSTANTS
          .AUTO_QUALITY_HIGHER_RESOLUTION_DELAY;
      }
